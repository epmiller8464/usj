...
public class KMSWebSocketChannel : NSObject , SignalingChannel, SRWebSocketDelegate{
	
	
	static let kMessageErrorKey : NSString = "error";
	static let kMessagePayloadKey :NSString = "sdpAnswer";
...
//		self._delegate?.channel(self, didStateChange:self._state!);
	...
	func registerWithCollider(){
		...
		let regMessage = ["action":"register","locationId": self.locationId , "id" : self.id];
		let msg = try? NSJSONSerialization.dataWithJSONObject(regMessage, options: NSJSONWritingOptions.PrettyPrinted);
		var msgStr =	 NSString(data: msg!, encoding: NSUTF8StringEncoding);
...
//			var message = ["locationId": self.locationId , "id" : self.id,"sdpOffer":payload!];
			//message.type
			let message = ["locationId": self.locationId , "id" : "viewer","sdp":payload!];
			let msgObj = try? NSJSONSerialization.dataWithJSONObject(message, options: NSJSONWritingOptions.PrettyPrinted);
...
//		var jsonObj : AnyObject? = NSJSONSerialization.JSONObjectWithData(msgData!, options: NSJSONReadingOptions.MutableContainers, error: nil);
		
		if jsonObj == nil {// &&  jsonObj! is NSDictionary? {
		...
		let mt = wssMessage[Utility.kSignalingMessageTypeKey] as? String;
		let errorString = wssMessage[KMSWebSocketChannel.kMessageErrorKey] as? String;
		...
		if mt == "answer" {
			messageKey = "sdpAnswer";
		}
		
		let payload = wssMessage[messageKey] as? String;
		var sm = SignalingMessage.messageFromJSONString(payload!);
		print("payload: \(payload)");
		let desc = RTCSessionDescription(type:"answer", sdp: payload! as String);
		let message = SessionDescriptionMessage(description: desc);
//		self._delegate!.channel(self, didReceiveMessage: message!);
	}
